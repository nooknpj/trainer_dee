...
(
    ...
    (200),
    SeenClientID varchar
    (13),
    PostedTrainerID varchar
    (13),
    
    primary key
    (CourseID),
    foreign key
    (SeenClientID) references Client
    (ClientID) on
    delete cascade on
    update cascade,
    foreign key(PostedTrainerID) references Client(ClientID)
    on
    delete cascade on
    update cascade
);
    ...
    (
        ReservationID int
        auto_increment,
    ApprovedTrainerID varchar
        (13),
    
    primary key
        (ReservationID),
    foreign key
        (ApprovedTrainerID) references Client
        (ClientID) on
        delete cascade on
        update cascade
);
...
(
    ...
    foreign key
            (locateCourseID) references Course
            (CourseID) on
            delete cascade on
            update cascade
);
            ...
            (
                RequestID int
                auto_increment,
    TrainerAcceptingID varchar
                (13),
    
    primary key
                (RequestID, TrainerAcceptingID),
    foreign key
                (TrainerAcceptingID) references Client
                (ClientID) on
                delete cascade on
                update cascade
);
    ...
    foreign key
                    (PaymentRequestID) references Request
                    (RequestID) on
                    delete cascade on
                    update cascade
);
